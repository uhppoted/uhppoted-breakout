...
void sys_tick() {
    ...
    debugf("SYS",
           "%-5u queue:%u  I2C0:%u  total heap:%u  free heap:%u  used:%.1f%%  "
           "errors:%04x  %s",
           counter, queue_get_level(&queue), queue_get_level(&I2C0.queue), heap,
           available, 100.0f * used, errors, watchdogged);
 ...
 *
 */
void sys_watchdog_update() { sys.touched = get_absolute_time(); }
